...
//2.开始查询
// 因为ID是唯一的 那么找到第一个满足条件的就不用再继续了 limit 1
...
function edit () {
	...
	if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
		...
		if (!move_uploaded_file($_FILES['avatar']['tmp_name'],$target)) {
			$GLOBALS['error_message'] = '上传头像文件失败';
			return;
		}
	...
	$query_update = mysqli_query($connection,"update users set name = '{$user['name']}',gender = {$user['gender']},birthday = '{$user['birthday']}',avatar = '{$user["avatar"]}' where id = {$id1};");
